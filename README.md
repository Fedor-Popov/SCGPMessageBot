...
The `/add` command asks for date, title, speaker, abstract, time, and location. Enter `-` for an optional blank field or `/cancel` to stop. Manual events are saved in `manual-events.json` and sent directly to the output spreadsheet before the slower source refresh runs. Every hourly source refresh also syncs newly discovered events to the output spreadsheet. Existing spreadsheet events are detected and are not added twice. New rows use the first available event area even when the sheet has blank checkbox rows farther down, so Google does not place an event at the bottom of the template. The startup and daily 1:00 AM exports remain as safeguards. Recurring Bouncing Seminar entries end before October 1, 2026.
