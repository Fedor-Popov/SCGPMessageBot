...
void sync_google_calendar(const std::vector<Event>& events) {
    ...
    for (const auto& event : events) if (!event.title.empty() || !event.description.empty()) call("event insertion", "https://www.googleapis.com/calendar/v3/calendars/" + encode(id) + "/events", calendar_body(event), auth); std::cerr << "Rebuilt the public Google Calendar with " << events.size() << " events\n";
}
